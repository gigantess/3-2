...
│  - 'data': 656건 주가 시계열 │  │  - gemini-3.6-flash (최신)   │
...
- **역할**: `gemini-3.6-flash` 최신 모델(자동 폴백: `gemini-flash-latest`, `gemini-3.5-flash`)을 활용하여, 주입된 Firestore 요약 데이터와 사용자의 질문 의도를 결합해 **사실(Fact) - 원인 분석(Why) - 전략적 조언(Action)** 구조로 완성도 높은 금융 답변 제공.
...
| | `POST /api/data/sync` | 오늘까지의 실시간 주가 데이터 외부 소스(Yahoo Finance) 자동 동기화 | **달성 (100%)** |
...
| | 데이터 관리 (CRUD) 화면 | 일별 주가 목록 테이블, 모달 창을 통한 새 데이터 추가 및 삭제/수정 동작, 1클릭 실시간 동기화 | **달성 (100%)** |
...
3. 사용자가 '내일 오를까?', '사도 될까?', '전망이 어때?'와 같은 방향성/시점 질문을 할 경우, '보합/횡보입니다'와 같은 단순한 단정이나 회피 대신:
   - [Fact] 현재 최신 종가({metrics.latest:,.0f}원)와 최근 20일 이동평균선/추세 위치 제시
   - [Why] 내일 주가 등락을 결정짓는 핵심 변수(미국 반도체 지수/엔비디아 흐름, 외국인/기관 수급 전환 여부, 환율) 분석
   - [Action] 단기 관점(지지선/저항선 확인)과 중장기 관점(분할 매수/손절 대응책)을 나누어 실전 조언을 제공하세요.
4. 사용자가 데이터 외적인 질문을 하더라도 삼성전자 주가 트렌드와 연계하여 답변을 유도하세요.
5. 존댓말과 정중한 어조를 유지하세요.
"""
```
...
- **지원 명령어 및 유연한 수신 트리거**:
  - `!질문 <질문내용>`, `!삼성 <질문내용>`, `!주가 <질문내용>`: 삼성전자 시계열 데이터를 기반으로 한 AI 맞춤 Fact-Why-Action 분석 답변을 임베드 카드로 답장
  - `@주식 비서 <질문내용>` (봇 멘션) 및 1:1 DM: 자유롭게 질문 시 즉시 맞춤 답변
  - 키워드 자동 감지: 메시지에 "삼성", "주가", "전망", "분석" 등이 포함되어 있을 경우 봇이 지능적으로 인지하여 금융 분석 응답 생성
  - `!브리핑`: 현재 주가 요약 및 기술적 보조지표(RSI, SMA, 변동성)/매매 신호 카드를 채널에 즉시 답장
- **Intent 폴백 및 안전성 보장**:
  - Discord 개발자 포털의 `MESSAGE CONTENT INTENT` 활성화 여부를 자동 감지하며, 미활성화 시 `discord.Intents.default()`로 안전하게 폴백 연결되어 봇이 비정상 종료되지 않고 지속 실행됩니다.
  - Windows 콘솔(`cp949`) 환경의 이모지/유니코드 출력 충돌을 차단하기 위해 `sys.stdout.reconfigure(encoding="utf-8")` 및 안전한 터미널 로깅이 적용되었습니다.
- **실행 방법**:
  ```powershell
  # 1. 단독 봇 데몬 실행
...
- **Discord Bot Token 2분 발급 가이드**:
  ...
  3. 동일 페이지 하단 **[Privileged Gateway Intents]**에서 **`MESSAGE CONTENT INTENT`** 활성화(ON 권장, 미활성화 시에도 자동 폴백 지원)
  4. 좌측 **[OAuth2]** → **[URL Generator]** → 스코프 `bot` 체크 → 권한 `Send Messages`, `Embed Links`, `Read Message History` 체크 후 생성된 URL로 봇을 내 디스코드 서버에 초대

---

### 11. 🚀 최근 주요 고도화 및 안정화 내역 ("디스코드 양방향 통신 구현" 커밋 이후)

`디스코드 양방향 통신을 위한 백엔드 구현 (commit: 4ea18fd)` 이후 실전 테스트 및 사용자 피드백을 통해 발견된 핵심 과제(인텐트 에러, 트리거 수신 제한, 구형 LLM 단종 및 모호한 답변)를 전면 해결하고 서비스를 대폭 고도화하였습니다:

| 커밋 해시 | 커밋 메시지 | 핵심 개선 사항 및 해결된 문제 |
| :--- | :--- | :--- |
| **`54c1d50`** | **디스코드 봇 Intent 폴백 및 인코딩 예외 처리 보완** | • **`PrivilegedIntentsRequired` 에러 해결**: Discord 개발자 센터에서 `Message Content Intent`를 켜지 않아도 `intents.default()`로 자동 폴백 연결 지원<br>• **Windows 콘솔 인코딩(`cp949`) 충돌 방지**: UTF-8 강제 재구성(`sys.stdout.reconfigure`) 및 유니코드/이모지 출력 예외 차단 |
| **`b8292ac`** | **디스코드 봇 수신 트리거 유연화 및 콘솔 실시간 로깅 추가** | • **수신 트리거 다변화**: 단순 `!질문` 접두사 외에도 `!삼성`, `!주가`, `@봇 멘션`, 1:1 다이렉트 메시지(DM), 메시지 본문 내 "삼성", "주가", "전망", "분석" 키워드 포함 시 자동 인식<br>• **콘솔 실시간 로깅**: 채널명, 작성자, 질문 내용, 답변 전송 완료 상태를 터미널 콘솔에 실시간 출력하여 모니터링 편의성 강화 |
| **`ed25a62`** | **Gemini 3.6-Flash 업그레이드 및 주가 방향성 예측 질의 맞춤형 프롬프트 강화** | • **Gemini 구형 모델 단종(404 Not Found) 해결**: `gemini-2.5-flash` 단종에 따른 API 오류를 해결하고 최신 정식 모델인 `gemini-3.6-flash`, `gemini-flash-latest` 멀티 모델 자동 폴백 체인 구축<br>• **"보합/횡보입니다" 회피성 응답 해결**: "내일 주가가 오를까?", "사도 될까?" 등 예측/시점 질문 시 **Fact(255,000원 최신가 및 이동평균선 위치) - Why(미국 반도체 지수, 외인 수급, 환율 3대 변수) - Action(단기 지지선/저항선 및 중장기 분할 매수 대응책)** 구조로 명확하게 답변하도록 프롬프트 전면 개편<br>• **Mock Fallback 고도화**: API 장애 시에도 질문 키워드 기반의 전문적인 대체 조언 반환 |

#### 💡 "내일 주가가 오를까?" 질문에 대한 답변 개선 전/후 비교

- **개선 전 (구형 모델 404 에러로 인한 기본 Mock 템플릿 출력)**:
  > *"현재 보유하신 삼성전자 데이터는 2024-01-02부터 2026-09-11까지 총 656건입니다. 최신 종가는 255,000원이며 전반적인 흐름은 보합/횡보입니다. ... 언제든 물어보세요!"*  
  > ❌ **문제점**: 질문자의 의도(내일 오를 것인가)에 대한 분석 없이 기계적인 횡보 안내만 반복됨.

- **개선 후 (`gemini-3.6-flash` + Fact-Why-Action 프롬프트 적용)**:
  > 1. **Fact (현재 위치)**: 최신 종가 255,000원은 20일 이동평균선(약 251,500원) 상단에 안착하여 단기 지지력을 테스트하는 구간입니다.
  > 2. **Why (내일 주가를 결정짓는 3대 변수)**:
  >    - 간밤 미국 뉴욕 증시의 필라델피아 반도체 지수 및 엔비디아 실적/주가 흐름
  >    - 최근 매도 우위였던 외국인 및 기관의 순매수 유입 전환 여부
  >    - 원/달러 환율의 1,380원선 안착 여부
  > 3. **Action (실전 투자 대응 전략)**:
  >    - **단기 관점**: 250,000원 지지선 유효 시 보유 및 260,000원 저항대 분할 익절 고려
  >    - **중장기 관점**: 섣부른 추격 매수보다는 248,000~250,000원 눌림목 구간에서 3~4회 분할 매수 대응 권장
  >  
  > ✅ **개선 효과**: 단순한 횡보 언급을 탈피하여 질문자가 즉시 실행할 수 있는 명확하고 전문적인 금융 인사이트 제공.

---

### 12. AI 도구 호출 (Function Calling) & MCP Server 연동 (보너스 과제 1)
